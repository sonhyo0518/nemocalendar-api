...
exports.fetchOgMeta = fetchOgMeta;
...
async function assertSafeHttpUrl(raw) {
    ...
    const records = await (0, promises_1.lookup)(hostname, { all: true });
...
async function fetchOgMeta(pageUrl) {
    ...
    try {
        ...
        const imageUrl = absUrl(finalUrl, metaContent(html, 'og:image') || metaContent(html, 'twitter:image'));
        const faviconUrl = absUrl(finalUrl, linkHref(html, 'icon') || linkHref(html, 'shortcut icon')) || absUrl(finalUrl, '/favicon.ico');
